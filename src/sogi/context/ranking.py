...
# Context has a different job at each point in the engineering lifecycle.
# Investigation needs a balanced map, implementation favors the relevant
# source/dependency surface, and verification/review deliberately raise tests
# and risk. Every profile sums to 1.0 so scores remain comparable.
_PHASE_WEIGHTS: dict[EngineeringPhase, tuple[float, float, float, float]] = {
    EngineeringPhase.UNDERSTAND: (0.50, 0.25, 0.10, 0.15),
    EngineeringPhase.INVESTIGATE: (0.40, 0.25, 0.20, 0.15),
    EngineeringPhase.PLAN: (0.35, 0.35, 0.15, 0.15),
    EngineeringPhase.IMPLEMENT: (0.45, 0.30, 0.10, 0.15),
    EngineeringPhase.VERIFY: (0.25, 0.15, 0.50, 0.10),
    EngineeringPhase.REVIEW: (0.20, 0.15, 0.35, 0.30),
    EngineeringPhase.DONE: (0.20, 0.15, 0.35, 0.30),
}
...
def rank_symbol(
    ...
    *,
    ...
    """Score a symbol for the task and current engineering phase."""
    ...
    semantic_weight, dependency_weight, test_weight, risk_weight = _PHASE_WEIGHTS[resolved_phase]
    score = (
